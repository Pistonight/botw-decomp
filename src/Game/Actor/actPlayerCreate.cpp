...
// ~BaseProcHandle() is discarded if it is defined in the header
namespace ksys::act {
inline BaseProcHandle::~BaseProcHandle() {
    deleteProc();
}
}  // namespace ksys::act
#endif
...
#ifdef MATCHING_HACK_NX_CLANG
    sead::Vector3f translation = player->getMtx().getTranslation();
    sead::Matrix34f pos = {
        1.0, 0.0, 0.0, translation.x, 0.0, 1.0, 0.0, translation.y, 0.0, 0.0, 1.0, translation.z,
    };
#else
    sead::Matrix34f pos;
...
void CreatePlayerEquipActorMgr::doRequestCreateArmorByRawLife(s32 slot_idx,
    ...
    ksys::act::ActorCreator::instance()->requestCreateActor(
        actor_name.cstr(), ksys::act::ActorHeapUtil::instance()->getBaseProcHeap(),
...
void CreatePlayerEquipActorMgr::doRequestCreateArmor(s32 slot_idx, const sead::SafeString& name,
...

    {
        ...
        doRequestCreateArmorByRawLife(slot_idx, name, pos, &params, 2);
    }
